...
// Function: dump_spef
void Timer::dump_spef(std::ostream& os, const std::vector<Net*> &nets) const {
  std::shared_lock lock(_mutex);
  _dump_spef(os, nets);
}

// Function: _dump_spef
void Timer::_dump_spef(std::ostream& os, const std::vector<Net*> &nets) const {
  ...
  for(const Net *net_p: nets) {
    ...
    if(auto rct = net.rct(); rct == nullptr) {
      ...
      os << "\n*D_NET " << net._name << ' ' << rct->total_ncap() << '\n';
